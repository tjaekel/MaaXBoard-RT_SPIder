 ...
 */
static void USB_HostMsdFatfsDisplayFileInfo(FILINFO *fileInfo, EResultOut out);

/*!
 * @brief list files and sub-directory in one directory, the function don't check all sub-directories recursively.
 */
static FRESULT USB_HostMsdFatfsListDirectory(const TCHAR *path, EResultOut out);
...
static void USB_HostMsdFatfsDisplayFileInfo(FILINFO *fileInfo, EResultOut out)
{
     ...
     * hide, 'S' - system) */
    print_log(out, "%s %c%c%c %30s - %8dB %4d-%2d-%2d %2d:%2d:%2d\r\n", (fileInfo->fattrib & AM_DIR) ? "dir" : "fil",
...
static FRESULT USB_HostMsdFatfsListDirectory(const TCHAR *path, EResultOut out)
{
    ...
    while (1)
    {
        ...
        outputLabel = 1;
        USB_HostMsdFatfsDisplayFileInfo(&fileInfo, out);
    }
    if (!outputLabel)
    {
    	print_log(out, "\r\n");
...
static void USB_HostMsdFatfsTest(usb_host_msd_fatfs_instance_t *msdFatfsInstance)
{
    ...
    fatfsCode = USB_HostMsdFatfsListDirectory((char const *)&driverNumberBuffer[0], DEBUG_OUT);
    if (fatfsCode)
    {
        USB_HostMsdFatfsTestDone();
        return;
    ...
    usb_echo("list root directory:\r\n");
    fatfsCode = USB_HostMsdFatfsListDirectory(_T("1:"), DEBUG_OUT);
    if (fatfsCode)
    {
        USB_HostMsdFatfsTestDone();
        return;
    ...
    fatfsCode = USB_HostMsdFatfsListDirectory(_T("1:/dir_1"), DEBUG_OUT);
    if (fatfsCode)
    {
        USB_HostMsdFatfsTestDone();
        return;
    ...
    fatfsCode = USB_HostMsdFatfsListDirectory(_T("."), DEBUG_OUT);
    if (fatfsCode)
    {
        USB_HostMsdFatfsTestDone();
        return;
...
int USBH_Available(void)
{
	return sDeviceAvailable;
}

int USBH_ListRootDirectory(char *f, EResultOut out)
{
	if ( ! f)
		USB_HostMsdFatfsListDirectory(_T("1:"), out);
	else
		USB_HostMsdFatfsListDirectory(f, out);
	return 1;
}

static char sBuf[80];

int USBH_PrintFile(char *f, EResultOut out)
{
    FRESULT fatfsCode;
    FIL file;
	...
	fatfsCode = f_open(&file, f, FA_READ);
	if (fatfsCode)
	{
		return 0;		/* error */
	}

	while (f_gets(sBuf, sizeof(sBuf) - 1, &file))
	{
		print_log(out, "%s", sBuf);
	}
